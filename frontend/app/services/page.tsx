...
export default function ServicesPage() {
  ...
  const load=useCallback(async()=>{
    ...
    try{
      const body=await request('?page=1&page_size=100&dir=asc');
  ...
  const remove=async()=>{if(!selected)return;setSaving(true);setError('');try{await request(`/${selected.id}`,{method:'DELETE'});setItems(current=>current.filter(item=>item.id!==selected.id));setNotice('Service deleted successfully.');setModal(null);}catch(cause){setError(cause instanceof Error?cause.message:'Unable to delete service.');}finally{setSaving(false);}};

  const visible=useMemo(()=>items.filter(item=>item.service.toLowerCase().includes(query.trim().toLowerCase())),[items,query]);
