...
for (const count of [1, 2, 3, 4, 5, 6, 45]) {
  ...
  for (const type of ['Multi-class', 'Up to 3 classes', 'Up to 5 classes']) {
    ...
    const expected = 30 + Array.from({ length: Math.max(0, count - bundle) }, (_, i) => bundle + i + 1 + 10).reduce((a, b) => a + b, 0);
    assert.equal(result.total_fee, expected, type + ' / ' + count);
    assert.equal(result.rows.length, 1 + Math.max(0, count - bundle));
    assert.equal(result.rows[0].class_to, Math.min(bundle, count));
...
assert.throws(() => calculate(base, 'b', 'Multi-class', 1, rates), /no published rate/); checks++;
assert.throws(() => calculate(base, 'a', 'Multi-class', 3, rates.filter(rate => rate.class_number !== 3)), /3rd class.*no published rate/); checks++;
assert.throws(() => calculate(base, 'a', 'Multi-class', 1, [...rates, rates[0]]), /multiple/); checks++;
assert.throws(() => calculate(base, 'a', 'Up to 3 classes', 3, rates.map(rate => rate.class_type === 'Up to 3 classes' && rate.class_number === 2 ? { ...rate, attorney_fee: 5 } : rate)), /inconsistent/); checks++;
...
assert.equal(calculate({ ...base, available: false }, 'a', 'Multi-class', 1, rates).total_fee, 30); checks++;
...
(async () => {
  ...
  assert.equal(result.official_fee, 48); assert.equal(result.attorney_fee, 40); assert.equal(result.vat, 4); assert.equal(result.class_count, 4); assert.equal(result.class_pricing_rows.length, 2); checks++;
  ...
  const printable = { exports: {}, React: require('react'), toPlainText: value => String(value || '') };
  ...
  assert.ok(markup.includes("per mark / selected classes")); assert.match(markup, /Up to 3 classes/); checks++;
